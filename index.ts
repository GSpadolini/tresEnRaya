...
const div: NodeListOf<HTMLDivElement> = document.querySelectorAll('div');
const ul: NodeListOf<HTMLUListElement> = document.querySelectorAll('ul');
const li: NodeListOf<HTMLLIElement> = document.querySelectorAll('li');
...
let pcXO: XO ='';
let jugadorXO: 'X' | 'O' | '';
type XO = typeof jugadorXO;
let ganador=0;
let empatador=0;
let empiezasTu=true;

const comprovacion = (elemento: number) => {
...
const comprovacionPC = (elemento:number, valorXO:XO) => {
...
const seleccionarXO = (tresCuatro: 3 | 4) => {
